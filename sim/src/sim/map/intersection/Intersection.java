...
public class Intersection implements Drawable {
	...
	private void init() {
		segments[0] = new Segment(new LineTrack(new Vector2D(straight + turn
				+ buffer + width / 2, 0), new Vector2D(straight + turn + buffer
				+ width / 2, straight)));
		...
		segments[1] = new Segment(new LineTrack(new Vector2D(straight + turn
				+ buffer + width / 2, straight), new Vector2D(straight + turn
				+ buffer + width / 2, straight + turn)));
		segments[0].linkSegment(2, segments[1]);
		segments[2] = new Segment(new SquareCurveTrack(new Vector2D(straight
				+ turn + buffer + width / 2, straight + turn), new Vector2D(
				straight + turn + buffer + width / 2, straight + turn + buffer
						+ width / 2), new Vector2D(straight + turn, straight
				+ turn + buffer + width / 2)));
